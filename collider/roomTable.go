...
func (rt *roomTable) send(rid string, srcID string, cmd string, msg string) error {
	rt.lock.Lock()
	defer rt.lock.Unlock()

	r := rt.roomLocked(rid)
	return r.send(srcID, cmd, msg)
}
